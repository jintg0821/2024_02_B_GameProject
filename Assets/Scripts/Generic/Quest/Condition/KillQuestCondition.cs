...
namespace MyGame.QuestSystem
{
    ...
    public class KillQuestCondition : IQuestCondition
    {
        ...
        public KillQuestCondition(string enemyType, int requiredKills)  //처치 퀘스트 조건 초기화 생성자
        {
            this.enemyType = enemyType;
            this.requiredKills = requiredKills;
            this.currKills = 0;
        } 
        public bool IsMet() => currKills >= requiredKills;      //목표 처치 수를 달성 했는지 확인
        public void Initialize() => currKills = 0;              //처치 수를 0으로 초기화
        public float GetProgress() => (float)currKills / requiredKills; //현재 처치 진행도를 퍼센트로 변환
        public string GetDescription() => $"Defeat {requiredKills} {enemyType} ({currKills}/{requiredKills})";  //퀘스트 조건 설명을 문자열로 변환
